...
MALE_HINTS = ("david", "george", "mark", "guy", "davis", "christopher", "male", "stefan", "james")
...
class TTSWorker(threading.Thread):
    def __init__(self, hub, rate: int = 185, voice: str = "", engine: str = ""):
        ...
        self.gender_pref = "feminine"
        self.voice_name = ""

    def set_voice(self, voice_name: str, gender: str = "") -> None:
        """Dynamically switch active TTS voice (e.g. when changing character persona)."""
        self.neural_voice = voice_name
        self.voice_pref = voice_name
        if gender:
        ...
        with self._engine_lock:
            if self._engine is not None:
                try:
                    self._pick_voice(self._engine)
                except Exception:
                    pass
        log.info("Switched TTS voice: neural=%s, gender=%s", voice_name, self.gender_pref)
        ...
        if self.voice_pref:
            for v in voices:
                if self.voice_pref.lower() in (v.name or "").lower():
                    ...
                    break
        if chosen is None:
            is_male = "masculine" in self.gender_pref.lower() or "male" in self.gender_pref.lower()
            target_hints = MALE_HINTS if is_male else FEMALE_HINTS
            for v in voices:
                name = (v.name or "").lower()
                gender = str(getattr(v, "gender", "") or "").lower()
                if any(h in name for h in target_hints) or (("male" in gender and "female" not in gender) if is_male else "female" in gender):
                    chosen = v
                    break
