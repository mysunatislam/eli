...
class SpeechController:
    def __init__(self, hub, settings, on_command):
        ...
        try:
            ...
            self.hotkey_listener = GlobalHotkeyListener(self.ptt_toggle)
            self.hotkey_listener.start()
        except Exception as e:
            log.warning("Could not start global hotkey listener: %s", e)
    ...
    def ptt_toggle(self) -> str:
        ...
        if self.recording and self._ptt_thread is not None:
            ...
            try:
                import winsound
                threading.Thread(target=lambda: (winsound.Beep(880, 60), winsound.Beep(587, 80)), daemon=True).start()
            except Exception:
                pass
            return "stopping"
        ...
        try:
            import winsound
            threading.Thread(target=lambda: (winsound.Beep(988, 70), winsound.Beep(1318, 90)), daemon=True).start()
        except Exception:
            pass
        self.hub.toast("Listening... (Press Ctrl+Space to stop)")
