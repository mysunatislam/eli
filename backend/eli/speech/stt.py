...
def normalize_speech_text(text: str) -> str:
    ...
    # 8. Fix VS Code acoustic mishearings ("gascode", "gas code", "js code", "j s code", "just code", "ds code", etc.)
    t = re.sub(r"\b(?:gas\s*code|gascode|gas\s*cod|gascod|guess\s*code|guesscode|ps\s*code|pscode|bs\s*code|bscode|vis\s*code|viscode|vez\s*code|vezcode|js\s*code|j\s+s\s*code|just\s*code|ds\s*code)\b", "VS Code", t, flags=re.I)
    t = re.sub(r"\b(?:the\s+)?vs\s+code\b", "VS Code", t, flags=re.I)

    # 9. Fix "keep submitting" / "auto submit" acoustic mishearings ("keep submit in", "keep sub meeting", "keep submiting", etc.)
    t = re.sub(r"\b(?:keep|continuous|continue)\s+(?:submiting|sub\s*meeting|submit\s*in|sub\s*mitting)\b", "keep submitting", t, flags=re.I)
    t = re.sub(r"\b(?:auto|automatic)\s+(?:submiting|sub\s*meeting|sub\s*mit)\b", "auto submit", t, flags=re.I)
