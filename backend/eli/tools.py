...
AUTOMATION_TOOLS = [
    ...
    _t("play_youtube", "Search and play music or a video on YouTube.",
       {"query": {"type": "string"}}, ["query"]),
    _t("switch_persona", "Switch Eli's active character persona, avatar appearance, and voice. Available personas: 'classic' (Eli Companion, feminine), 'agency' (Atlas Tech & Engineering, masculine), 'hotel' (Aria Hospitality Concierge, refined feminine), 'event' (Zephyr Event Coordinator, dynamic masculine), 'mentor' (Mentor Procedural Guide & Tutor, instructional masculine).",
       {"persona": {"type": "string", "description": "The persona ID or name to switch to"}}, ["persona"]),
...
MEMORY_TOOLS = [
    ...
    _t("index_knowledge", "Index a file or directory into Ellie's local vector knowledge base for RAG retrieval.",
       {"path": {"type": "string", "description": "Absolute path to a file (.md, .py, .txt, .json, etc.) or folder to index."},
        "category": {"type": "string", "description": "Optional category (e.g. codebase, documentation, notes)"}}, ["path"]),
    _t("search_knowledge_base", "Perform a semantic vector search across all indexed documents, code files, and knowledge base chunks.",
       {"query": {"type": "string", "description": "What to search for."},
        "limit": {"type": "integer", "description": "Maximum number of chunks to return (default: 5)"}}, ["query"]),
...
# --- Teach-me: learn a guide by watching the user once -------------------------------------------------------
TEACH_TOOLS = [
    _t("start_recording", "Teach-me mode: start watching the user's own clicks and keystrokes so Eli can learn a new guide "
       "from ONE demonstration. Use when the user says 'let me teach you how to…', 'watch me…', 'record this', 'learn this from me'. "
       "Never records on block-listed apps or in private mode; typed text is stored as a character count unless the user enabled "
       "teach_capture_text. Tell the user to do the task once and say 'stop recording' when done.",
       {"name": {"type": "string", "description": "Short name for the guide, e.g. 'export a STEP file'."},
        "goal": {"type": "string", "description": "What the guide achieves, in the user's words."}}, ["name"]),
    _t("stop_recording", "Stop the teach-me recording and turn it into a runnable guide (segmented, validated, saved to the "
       "library and registered so start_guide can run it). Pass run_now=true to guide the user through it immediately.",
       {"run_now": {"type": "boolean"}}),
    _t("list_guides", "List the guides Eli has learned (recorded or imported).", {}),
    _t("review_guide", "Show the steps of a learned guide in plain language so the author can check or fix them.",
       {"guide_id": {"type": "string"}, "name": {"type": "string"}}),
    _t("edit_guide_step", "Fix one step of a learned guide: rename it, rewrite its instruction or narration, or delete it.",
       {"index": {"type": "integer", "description": "1-based step number"}, "guide_id": {"type": "string"},
        "title": {"type": "string"}, "instruction": {"type": "string"}, "say": {"type": "string"}, "delete": {"type": "boolean"}},
       ["index"]),
    _t("rename_guide", "Rename a learned guide.", {"name": {"type": "string"}, "guide_id": {"type": "string"}}, ["name"]),
    _t("delete_guide", "Delete a learned guide.", {"guide_id": {"type": "string"}, "name": {"type": "string"}}),
    _t("export_guide", "Export a learned guide to a shareable .eliguide.json file (no screenshots, no recording ids).",
       {"guide_id": {"type": "string"}, "name": {"type": "string"}, "path": {"type": "string"}}),
    _t("import_guide", "Import a guide from a .eliguide.json file someone shared, and register it so it can be run.",
       {"path": {"type": "string"}}, ["path"]),
]

TOOLS: list[dict] = VISION_TOOLS + AUTOMATION_TOOLS + CODING_TOOLS + DESIGN_TOOLS + COMMUNICATION_TOOLS + MEMORY_TOOLS + TEACH_TOOLS
...
def describe_action(name: str, args: dict) -> str:
    ...
    if name == "start_recording":
        return "start learning a guide from you: " + str(a.get("name") or a.get("goal") or "")
    if name == "stop_recording":
        return "stop recording and write the guide"
    if name in ("review_guide", "list_guides", "export_guide"):
        return name.replace("_", " ")
    if name == "delete_guide":
        return f"delete the guide {a.get('name') or a.get('guide_id') or ''}"
    if name == "import_guide":
        return f"import a guide from {a.get('path', '')}"
    ...
    if name == "play_youtube":
        return f"play \"{a.get('query', '')}\" on YouTube"
    if name == "switch_persona":
        return f"switch avatar persona to {a.get('persona', '')}"
    if name == "open_ide":
