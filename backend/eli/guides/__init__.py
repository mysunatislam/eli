...
BUILTIN_IDS = frozenset(WORKFLOWS)


def register(wf: dict) -> None:
    """Add a learned (recorded or imported) workflow so the engine and find_workflow can use it."""
    WORKFLOWS[wf["id"]] = wf


def unregister(wf_id: str) -> None:
    if wf_id not in BUILTIN_IDS:
        WORKFLOWS.pop(wf_id, None)


def find_workflow(text: str):
    """Match a user request to a workflow: built-ins by keyword groups first, then learned guides by name."""
    low = (text or "").lower()
    for wf_id in BUILTIN_IDS:
        wf = WORKFLOWS[wf_id]
        groups = wf.get("keywords", [])
        if groups and all(any(k in low for k in group) for group in groups):
            return wf
    # learned guides: exact name first, then their keyword group (any word), longest name wins ties
    learned = [wf for k, wf in WORKFLOWS.items() if k not in BUILTIN_IDS]
    for wf in sorted(learned, key=lambda w: -len(w.get("name", ""))):
        if wf.get("name") and wf["name"].lower() in low:
            return wf
    for wf in learned:
        groups = wf.get("keywords", [])
        if groups and all(any(k in low for k in group) for group in groups):
