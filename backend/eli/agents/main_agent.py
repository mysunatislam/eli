...
from ..personas import Persona, get_persona, find_persona_by_query, list_personas, PERSONAS
...
BASE_PERSONA_GUIDELINES = """You are direct, lightning-fast, and exceptionally capable: a brilliant pair-assistant who bridges gaps and takes action. You are persistent: you remember the user across sessions and you act, not just answer.
...
- CONTROL: open_app, open_url, web_search, play_youtube, switch_persona, focus_window, type_text, press_keys, click, click_text, scroll, wait.
...
- TEACH: when the user offers to show you how something is done ("let me teach you…", "watch me…", "record this"), call start_recording with a short name, tell them to do it once and say "stop recording"; on "stop recording" call stop_recording. Learned guides run through start_guide by name. list_guides / review_guide / edit_guide_step / rename_guide / delete_guide / export_guide / import_guide manage them. Prefer a learned guide over planning a new one when the names match.
...
Rules:
- Act immediately on clear commands; bridge the gaps proactively. When asked to play music, play it on YouTube.
- When asked to switch character persona, avatar appearance, or voice, call switch_persona.
...
PERSONA = BASE_PERSONA_GUIDELINES
...
class ToolExecutor:
    ...
    def __init__(self, vision, auto, memory, coding, design, comms, hub, settings, broker: PermissionBroker):
        ...
        self.teach = None  # TeachAgent, attached by the server
        self.scheduler = None  # Scheduler, attached by the server
        self.persona_switcher = None  # switch_persona callback, attached by MainAgent
    ...
    async def execute(self, call: ToolCall) -> ToolResult:
        ...
        if name == "start_recording" and self.teach is not None:
            return ToolResult(await self.teach.start(str(args.get("name", "")), str(args.get("goal", ""))))
        if name == "stop_recording" and self.teach is not None:
            return ToolResult(await self.teach.stop(run_now=bool(args.get("run_now"))))
        if name == "list_guides" and self.teach is not None:
            return ToolResult(self.teach.list_guides())
        if name == "review_guide" and self.teach is not None:
            return ToolResult(self.teach.review(str(args.get("guide_id", "")), str(args.get("name", ""))))
        if name == "edit_guide_step" and self.teach is not None:
            return ToolResult(self.teach.edit_step(int(args.get("index") or 0), str(args.get("guide_id", "")), str(args.get("title", "")),
                                                   str(args.get("instruction", "")), str(args.get("say", "")), bool(args.get("delete"))))
        if name == "rename_guide" and self.teach is not None:
            return ToolResult(self.teach.rename(str(args.get("name", "")), str(args.get("guide_id", ""))))
        if name == "delete_guide" and self.teach is not None:
            return ToolResult(self.teach.delete(str(args.get("guide_id", "")), str(args.get("name", ""))))
        if name == "export_guide" and self.teach is not None:
            return ToolResult(self.teach.export(str(args.get("guide_id", "")), str(args.get("path", "")), str(args.get("name", ""))))
        if name == "import_guide" and self.teach is not None:
            return ToolResult(self.teach.import_file(str(args.get("path", ""))))
        if name == "guide_control" and self.guide is not None:
            return ToolResult(await self.guide.control(str(args.get("action", ""))) or "ok")
        if name == "switch_persona" and self.persona_switcher is not None:
            pid = str(args.get("persona", ""))
            return ToolResult(await self.persona_switcher(pid))
...
class MainAgent:
    def __init__(self, hub, settings, memory, vision, auto, coding, design, comms, llm, broker: PermissionBroker, proactive=None):
        ...
        self._pending_code_fix = None
        self.active_persona = get_persona(self.settings.get("active_persona", "classic"))
        self.executor.persona_switcher = self.switch_persona

    def attach_speech(self, speech) -> None:
        self.speech = speech
        if self.speech and hasattr(self.speech, "tts") and self.speech.tts:
            self.speech.tts.set_voice(self.active_persona.voice, gender=self.active_persona.gender)

    async def switch_persona(self, query_or_id: str) -> str:
        persona = find_persona_by_query(query_or_id) or get_persona(query_or_id)
        self.active_persona = persona
        self.settings.set("active_persona", persona.id)
        if self.speech and hasattr(self.speech, "tts") and self.speech.tts:
            self.speech.tts.set_voice(persona.voice, gender=persona.gender)
        self.hub.emit({
            ...
            "persona": {
                ...
                "voice": persona.voice,
                "color": persona.color_accent,
                "color_light": persona.color_light,
                "color_dark": persona.color_dark,
                "shape": persona.shape,
            }
        })
        log.info("Switched active persona to %s (%s)", persona.display, persona.voice)
        return f"I am now {persona.display}. {persona.tagline}."
    ...
    async def _route(self, text: str, source: str) -> str:
        # Check if Eli was waiting for confirmation to fix code error in VS Code
        if self._pending_code_fix:
            fix_info = self._pending_code_fix
            low = text.lower().strip().strip(".!?,")
            # 1. Negative cancellation check FIRST
            if any(re.search(rf"\b{re.escape(k)}\b", low) for k in ("no", "don't", "dont", "leave", "cancel", "never mind", "nevermind", "stop", "abort", "not now", "skip")):
                self._pending_code_fix = None
                self.coding.pending_code_fix = None
                if self.speech and hasattr(self.speech, "wake") and self.speech.wake:
                    self.speech.wake.enter_standby()
                return "Understood. I left your code as-is."

            # 2. Positive confirmation
            if any(re.search(rf"\b{re.escape(k)}\b", low) for k in ("yes", "fix", "please", "go ahead", "do it", "sure", "okay", "ok", "yep", "yeah", "proceed", "fix it")):
                self._pending_code_fix = None
                self.coding.pending_code_fix = None
                return await asyncio.to_thread(self.coding.apply_visible_code_fix, self.auto, fix_info)
    ...
    async def _run_intent(self, intent, raw: str) -> Optional[str]:
        ...
        if kind == "write_code_vscode":
            ...
            if self.speech and hasattr(self.speech, "wake") and self.speech.wake:
                self.speech.wake.enter_standby()
            return res_msg
        if kind == "switch_persona":
            target = arg or raw
            return await self.switch_persona(target)
        if kind == "teach_start" and self.executor.teach is not None:
            return await self.executor.teach.start(arg, arg)
        if kind == "teach_stop" and self.executor.teach is not None:
            return await self.executor.teach.stop(run_now=False)
        if kind == "teach_cancel" and self.executor.teach is not None:
            return (await self.executor.teach.cancel()) or "I wasn't recording."
        if kind == "teach_list" and self.executor.teach is not None:
            return self.executor.teach.list_guides()
        if kind == "teach_review" and self.executor.teach is not None:
            return self.executor.teach.review(text=arg)
        if kind == "teach_export" and self.executor.teach is not None:
            return self.executor.teach.export(text=arg)
        if kind == "greeting":
            if self.speech and hasattr(self.speech, "wake") and self.speech.wake:
                self.speech.wake.extend_conversation(60.0)
        ...
        if kind == "vscode_check_code":
            target_fn = groups[0] if groups else ""
            res = await asyncio.to_thread(self.coding.inspect_and_diagnose_vscode, self.auto, self.vision, target_fn)
            if self.coding.pending_code_fix:
                self._pending_code_fix = self.coding.pending_code_fix
                if self.speech and hasattr(self.speech, "wake") and self.speech.wake:
                    self.speech.wake.extend_conversation(60.0)
            return res
        if kind == "learn_3d":
        ...
        if kind == "stop_all":
            ...
            if self.executor.guide:
                try:
                    ...
                    pass
            if self.executor.teach:
                try:
                    await self.executor.teach.cancel()
                except Exception:
                    pass
    ...
    def _build_system_prompt(self) -> str:
        ...
        header = (
            f"=== CURRENT AVATAR: {p.display} ===\n"
            f"Persona & Tone: {p.system_prompt}\n"
            f"Tagline: {p.tagline}\n"
            f"Voice Profile: {p.voice} ({p.gender})\n\n"
        )
        return header + BASE_PERSONA_GUIDELINES
    ...
    def _system(self) -> tuple[str, str]:
        ...
        return self._build_system_prompt(), dynamic
    ...
    async def _execute_offline_instruction(self, text: str) -> Optional[str]:
        ...
        # 1. Deterministic intents (media, 3d, ide, teach, persona switch, etc.)
        ...
        if any(k in low for k in ("read the whole script", "read the script", "check my code", "check the code", "check this code", "check the script", "check my script", "diagnose code", "find errors in my code", "find errors in code", "specifically identify the errors", "check for errors", "check how it is", "how it is")) or (
            any(k in low for k in ("vs code", "vscode", "in here", "the code i have written", "code i wrote", "editor")) and any(k in low for k in ("check", "inspect", "analyse", "analyze", "see", "error", "errors", "look", "how"))
        ):
            target_fn = ""
            fn_m = re.search(r"\b([a-zA-Z0-9_\-]+\.py)\b", raw, re.I)
            if fn_m:
                target_fn = fn_m.group(1)
            res = await asyncio.to_thread(c.inspect_and_diagnose_vscode, a, v, target_fn)
            if c.pending_code_fix:
                self._pending_code_fix = c.pending_code_fix
                if self.speech and hasattr(self.speech, "wake") and self.speech.wake:
                    self.speech.wake.extend_conversation(60.0)
            return res
