...
```powershell
git clone https://github.com/mysunatislam/eli.git C:\eli      # keep path short (Windows 260-char path limit)
cd C:\eli
...
## Teach-me mode (0.5): show Eli once, and it can guide anyone

Say *"Eli, let me teach you how to export a STEP file"* (or *"watch me …"*, *"record this"*), do the task once at your
normal pace, then say *"stop recording"*. Eli turns what it saw into a guide in the same step schema as the built-in
walkthroughs, so *"guide me through exporting a STEP file"* now works in that app with on-screen arrows, narration and
auto-advance — for you, or for anyone you send the guide to.

**What gets recorded.** Your own clicks, double-clicks, right-clicks and keyboard shortcuts (Windows low-level hooks;
events injected by software are ignored, so Eli never records itself). Around every action Eli keeps: the OCR label under
the cursor, the active window before/after, and which on-screen texts appeared or disappeared — that is where each
step's *target* (what to point at) and *expect* (how to tell it's done) come from. Mouse movement is never recorded.
Plain typing is stored as a **character count only**, unless you set `"teach_capture_text": true` in settings.
Private mode refuses to record; block-listed apps are never captured. Raw traces live under `data\recordings\<id>\`
(`trace.json` plus small marked-up JPEGs) and can be deleted at any time.

**How the guide is written.** With a vision model configured, Eli sends the trace plus up to six marked-up screenshots
and asks for a learner-friendly walkthrough (merged logical steps, narration that explains *why*), then validates it the
same way dynamic plans are validated and **back-fills** any target or completion condition the model left vague from the
recorded evidence. Offline, a deterministic segmenter builds one step per action. Either way the result is saved in
`data\guides\<id>.json` and registered with the engine.

| Say | What Eli does |
|---|---|
| *"let me teach you how to …"*, *"watch me …"*, *"record this"* | Starts recording (asks for screen permission once). The panel shows a purple **learning · N actions** pill and a toast per action. |
| *"stop recording"*, *"I'm done with the demo"* | Stops, writes the guide, saves it, tells you how to run it. |
| *"cancel the recording"* | Discards it. |
| *"what guides do you have?"*, *"review that guide"* | Lists learned guides / reads back the steps with their targets and completion checks. |
| *"rename that guide to …"*, *"delete step 3"*, *"change step 2's instruction to …"* | Small edits by voice (tools `rename_guide`, `edit_guide_step`, `delete_guide`). |
| *"export that guide"* / *"import guide C:\path\x.eliguide.json"* | Shares a self-contained `.eliguide.json` (no screenshots, no typed text unless you kept it) / installs one from a colleague, a course, or a vendor. |

Tests: `backend\tests\test_teach.py` (pytest; runs on any OS — trace model, segmenters, library, lookup, intents).
Design notes and the roadmap for this feature: `docs/ELI_STRATEGY_AND_TEACH_ME.md`.
